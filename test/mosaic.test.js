...
describe('ImageModel Constructor ', function() {
    ...
    it('should throw an error if original image is not defined', function() {
      expect(function(){new app.ImageModel()}).to.throw('source image not defined');
    });    

});

describe('Mosaic Worker', function() {

  it('should exist', function(){
    expect(mosaicWorker).to.exist;
  });
  ...
  it('should correctly compute average rgb', function(){
    ...
    expect(getTileAvgRGB(0, 0, 2, 2, 2, imageData)).to.eql(avgRGB);
  });    
  ...
  it('should correctly compute hex from rgb', function(){
    ...
    expect(rgbToHex(rgb1)).to.eql('331ecc');
    expect(rgbToHex(rgb2)).to.eql('9b1ecc');
  });
