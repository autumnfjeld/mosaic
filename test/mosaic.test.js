...
describe('ImageModel Constructor ', function() {
  ...
  describe('initilization', function(){
    ...
    image.src = getSource();
    image.complete = true;  //not working
    // debugger;
    var model = new app.ImageModel(image);
    console.log('mooooodle', model);

    it('should accept an image', function(){
      expect(model.srcImage.imgEl).to.eql(image);
    });

    it('should throw an error if original image is not defined', function() {
      console.log('**@@@*', app.ImageModel);
      // var model2 = new app.ImageModel();
      // expect(model2).to.throw(TypeError);
      expect(function(){new app.ImageModel()}).to.throw('Error: source image not defined');
    });    
...
//     var image = new Image(),
//         test = document.getElementById('test-image');    
//     image.crossOrigin = 'Anonymous';
//     image.src = getSource();
//     //verify image: ie test the test
...
//     it('should accept an image', function(){
//       console.log('mosaid in tst', mosaic);
